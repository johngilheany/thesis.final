...
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-1.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-2.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-3.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-4.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-5.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-6.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-7.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-8.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-9.pdf}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-33-10.pdf}
...
\end{figure}
\section{Price to Book Ratios}\label{price-to-book-ratios}

Data was collected from 10/31/2010 to 12/30/2016, from Wharton Research
Data Services (WRDS) for the 908 historical constituents of the USA
Equal Weight (EUSA) ETF, of which the Minimum Volatility Index is
derived. Each tickers' Price to Book ratio was calculated, and a month
end spagetti plot was produced for stocks in the Minimum Volatility
Index, and the remainder of the stock comprising the US Equal Weight
Index, to get a relative sense of each group's volatility attributes.
The stocks comprising the Minimum Volatility Index generally had a
similar price to book ratio when compared those in the US Equal Weight
Index.
\begin{verbatim}
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 0.2829  1.6559  2.6368  4.1138  4.3370 96.1338 
\end{verbatim}
\begin{figure}[htbp]
\centering
\includegraphics{thesis_files/figure-latex/unnamed-chunk-43-1.pdf}
\caption{\label{fig:unnamed-chunk-43}The price to book ratios for stocks in
the US Equal Weight Index ranged from 0.2829 to 96.1338, with a mean
value of 4.1138}
\end{figure}
\begin{verbatim}
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 0.2829  2.0196  3.2208  5.0907  5.5060 76.4406 
\end{verbatim}
\begin{figure}[htbp]
\centering
\includegraphics{thesis_files/figure-latex/unnamed-chunk-45-1.pdf}
\caption{\label{fig:unnamed-chunk-45}The price to book ratios for stocks in
the Minimum Volatility Index ranged from 0.2829 to 76.4406, with a mean
value of 5.0907}
\end{figure}
...
Since the index is rebalanced twice a year (once in November and once in
May), it makes sense to look at a model for each of these individual
months. Since the dataset to test this model will be from May 2017, a
subset of the data was taken for May.
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Subset data for dates from May only}
\NormalTok{may_final <-}\StringTok{ }\KeywordTok{filter}\NormalTok{(monthly_data, date ==}\StringTok{ "2012-05-31"} \NormalTok{|}\StringTok{  }\NormalTok{date ==}\StringTok{ "2013-05-31"}\NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2014-05-30"} \NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2015-05-29"} \NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2016-05-31"}\NormalTok{)}
\CommentTok{# Remove NA values from set}
\NormalTok{may_final <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(may_final, !}\KeywordTok{is.na}\NormalTok{(index_before))}
\end{Highlighting}
\end{Shaded}
\subsection{Data Cleaning - Checking for Class
Bias}\label{data-cleaning---checking-for-class-bias}
...
that this is not the case, as, just around 24\% of the data is from
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(may_final$index_now)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(trainingData1$index_now)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
\begin{Highlighting}[]
\CommentTok{# Model 1}
\NormalTok{logit1 <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(index_now ~}\StringTok{  }\NormalTok{volatility +}\StringTok{ }\NormalTok{beta +}\StringTok{ }\NormalTok{price_to_book +}\StringTok{ }\NormalTok{index_before, }\DataTypeTok{data=}\NormalTok{trainingData1, }\DataTypeTok{family=}\KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link=}\StringTok{"logit"}\NormalTok{))}
...
\end{verbatim}
Looking at the May model will be helpful for someone looking to predict
index rebalancing between December and April.

\subsection{Interpretation of Model}\label{interpretation-of-model}

The model can be interpreted as: \hfill\break
ln{[}\(\frac{p}{1-p}\){]} = -1.74 - 0.04 x vol - 0.64 x beta - 0.012 x
price\_to\_book + 7.014 x index\_before \hfill\break
\(\frac{p}{1-p}\) = exp(-1.74 - 0.04 x vol - 0.64 x beta - 0.012 x
price\_to\_book + 7.014 x index\_before ) \hfill\break
...
- Beta: The odds ratio of being added to the index is 0.52 times
smaller, given a one unit increase in beta. This response variable is
statistically significant. \hfill\break
- Price to Book: The odds ratio of being added to the index is 0.99
...
- Index before: The odds ratio of being added to the index is 1112.01
...
\emph{Predictive Power} \hfill\break
...
\NormalTok{predicted1 <-}\StringTok{ }\KeywordTok{plogis}\NormalTok{(}\KeywordTok{predict}\NormalTok{(logit1, testData1)) }
\NormalTok{optCutOff1 <-}\StringTok{ }\KeywordTok{optimalCutoff}\NormalTok{(testData1$index_now, predicted1)[}\DecValTok{1}\NormalTok{] }
\NormalTok{optCutOff1}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.8487479
\end{verbatim}
\hfill\break
\emph{VIF}**\_ \hfill\break
Like in case of linear regression, we should check for multicollinearity
in the model. As seen below, all X variables in the model have VIF well
below 4.
\begin{Shaded}
\begin{Highlighting}[]
...
\end{verbatim}
\hfill\break
\emph{Misclassification Error} \hfill\break
...
error, the better the model. Here it is 2.3\%, which is quite low, and
good.
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{predicted1 <-}\StringTok{ }\KeywordTok{plogis}\NormalTok{(}\KeywordTok{predict}\NormalTok{(logit1, testData1)) }
\KeywordTok{misClassError}\NormalTok{(testData1$index_now, predicted1)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.0234
...
curve, better the predictive ability of the model. Here, it is 98.2\%.
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plotROC}\NormalTok{(testData1$index_now, predicted1)}
\end{Highlighting}
\end{Shaded}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-54-1.pdf}
\hfill\break
\emph{Concordance} \hfill\break
...
better is the quality of model. This model with a concordance of 98.2\%
is a good quality model.
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{Concordance}\NormalTok{(testData1$index_now, predicted1)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
$Concordance
...
\end{verbatim}
\hfill\break
\emph{Specificity and Sensitivity} \hfill\break
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{sensitivity}\NormalTok{(testData1$index_now, predicted1, }\DataTypeTok{threshold =} \NormalTok{optCutOff1)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.9059406
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{specificity}\NormalTok{(testData1$index_now, predicted1, }\DataTypeTok{threshold =} \NormalTok{optCutOff1)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.9859499
\end{verbatim}
\hfill\break
\emph{Confusion Matrix} \hfill\break
In the confusion matrix, the columns are actuals, while rows are
predicteds
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confusionMatrix}\NormalTok{(testData1$index_now, predicted1, }\DataTypeTok{threshold =} \NormalTok{optCutOff1)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
     0   1
...
Since the index is rebalanced twice a year (once in November and once in
May), it makes sense to look at a model for both of these months. Thus,
a subset of the data was taken for May and November, by combining the
data sets from Model 2 and Model 3.
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Subset data for dates from May and November}
\NormalTok{november_final <-}\StringTok{ }\KeywordTok{filter}\NormalTok{(monthly_data, date ==}\StringTok{ "2011-11-30"} \NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2012-11-30"}\NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2013-11-29"}\NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2014-11-28"} \NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2015-11-30"} \NormalTok{|}\StringTok{ }\NormalTok{date ==}\StringTok{ "2016-11-30"}\NormalTok{)}
\CommentTok{# Remove NA values from set}
\NormalTok{november_final <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(november_final, !}\KeywordTok{is.na}\NormalTok{(index_before))}

\NormalTok{both_final <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(may_final, november_final)}
\end{Highlighting}
\end{Shaded}
\subsection{Data Cleaning - Checking for Class
Bias}\label{data-cleaning---checking-for-class-bias-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(both_final$index_now)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
\end{verbatim}
\subsection{Create Training and Test
Samples}\label{create-training-and-test-samples-1}
...
\NormalTok{input_ones2 <-}\StringTok{ }\NormalTok{both_final[}\KeywordTok{which}\NormalTok{(both_final$index_now ==}\StringTok{ }\DecValTok{1}\NormalTok{), ] }
\NormalTok{input_zeros2 <-}\StringTok{ }\NormalTok{both_final[}\KeywordTok{which}\NormalTok{(both_final$index_now ==}\StringTok{ }\DecValTok{0}\NormalTok{), ]}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{100}\NormalTok{)  }\CommentTok{# for repeatability of samples}
\NormalTok{input_ones_training_rows2 <-}\StringTok{ }\KeywordTok{sample}\NormalTok{(}\DecValTok{1}\NormalTok{:}\KeywordTok{nrow}\NormalTok{(input_ones2), }\FloatTok{0.7}\NormalTok{*}\KeywordTok{nrow}\NormalTok{(input_ones2)) }
\NormalTok{input_zeros_training_rows2 <-}\StringTok{ }\KeywordTok{sample}\NormalTok{(}\DecValTok{1}\NormalTok{:}\KeywordTok{nrow}\NormalTok{(input_zeros2), }\FloatTok{0.7}\NormalTok{*}\KeywordTok{nrow}\NormalTok{(input_ones2))  }
\NormalTok{training_ones2 <-}\StringTok{ }\NormalTok{input_ones2[input_ones_training_rows2, ]  }
\NormalTok{training_zeros2 <-}\StringTok{ }\NormalTok{input_zeros2[input_zeros_training_rows2, ]}
\NormalTok{trainingData2 <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(training_ones2, training_zeros2)   }
\CommentTok{# Create Test Data}
\NormalTok{test_ones2 <-}\StringTok{ }\NormalTok{input_ones2[-input_ones_training_rows2, ]}
\NormalTok{test_zeros2 <-}\StringTok{ }\NormalTok{input_zeros2[-input_zeros_training_rows2, ]}
\NormalTok{testData2 <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(test_ones2, test_zeros2) }
\end{Highlighting}
\end{Shaded}
There is no more class bias, as the sample is now evenly weighted now,
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(trainingData2$index_now)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
\begin{Highlighting}[]
\CommentTok{# Model 2}
\NormalTok{logit2 <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(index_now ~}\StringTok{  }\NormalTok{volatility +}\StringTok{ }\NormalTok{beta +}\StringTok{ }\NormalTok{price_to_book +}\StringTok{ }\NormalTok{index_before, }\DataTypeTok{data=}\NormalTok{trainingData2, }\DataTypeTok{family=}\KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link=}\StringTok{"logit"}\NormalTok{))}
...
\end{verbatim}
Looking at this model will be helpful for someone looking to predict
index rebalancing, generally, for both months.

\subsection{Interpretation of Model}\label{interpretation-of-model-1}

The model can be interpreted as: \hfill\break
ln{[}\(\frac{p}{1-p}\){]} = -1.84 + 0.003 x vol - 0.31 x beta - 0.0019 x
price\_to\_book + 5.89 x index\_before \hfill\break
\(\frac{p}{1-p}\) = exp(-1.84 + 0.003 x vol - 0.31 x beta - 0.0019 x
price\_to\_book + 5.89 x index\_before) \hfill\break
The coefficients can be interpreted as: \hfill\break
- Volatility: The odds ratio of being added to the index is 1.0029 times
greater, given a one unit increase in volatility. This response variable
is not statistically significant. \hfill\break
- Beta: The odds ratio of being added to the index is 0.73 times
smaller, given a one unit increase in beta. This response variable is
statistically significant. \hfill\break
- Price to Book: The odds ratio of being added to the index is 0.99
times smaller, given a one unit increase in price to book ratio. This
response variable is not statistically significant. \hfill\break
- Index before: The odds ratio of being added to the index is 360.28
times greater if the stock was in the index 6 months ago. This response
variable is statistically significant. \hfill\break
\hfill\break
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(InformationValue)}
\NormalTok{predicted2 <-}\StringTok{ }\KeywordTok{plogis}\NormalTok{(}\KeywordTok{predict}\NormalTok{(logit2, testData2)) }
\NormalTok{optCutOff2 <-}\StringTok{ }\KeywordTok{optimalCutoff}\NormalTok{(testData2$index_now, predicted2)[}\DecValTok{1}\NormalTok{] }
\end{Highlighting}
\end{Shaded}
\hfill\break
\emph{VIF}**\_ \hfill\break
...
error, the better the model. Here it is 4.0\%, which is quite low, and
good.
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{predicted2 <-}\StringTok{ }\KeywordTok{plogis}\NormalTok{(}\KeywordTok{predict}\NormalTok{(logit2, testData2)) }
\KeywordTok{misClassError}\NormalTok{(testData2$index_now, predicted2)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.0402
...
steeply, indicating that the TPR (Y-Axis) increases faster than the FPR
(X-Axis) as the cutoff score decreases. Greater the area under the ROC
curve, better the predictive ability of the model. Here, it is 96.8\%.
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plotROC}\NormalTok{(testData2$index_now, predicted2)}
\end{Highlighting}
\end{Shaded}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-66-1.pdf}
\hfill\break
\emph{Concordance} \hfill\break
Ideally, the model-calculated-probability-scores of all actual
Positive's, (aka Ones) should be greater than the
...
positive's are greater than the scores of actual negative's. For a
perfect model, this will be 100\%. So, the higher the concordance, the
better is the quality of model. This model with a concordance of 96.8\%
is a good quality model.
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{Concordance}\NormalTok{(testData2$index_now, predicted2)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
$Concordance
...
86.3\%. \hfill\break
- Specificity can also be calculated as 1 - False Positive Rate. In this
model, it was found to be 98.2\%.
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{sensitivity}\NormalTok{(testData2$index_now, predicted2, }\DataTypeTok{threshold =} \NormalTok{optCutOff2)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.8629808
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{specificity}\NormalTok{(testData2$index_now, predicted2, }\DataTypeTok{threshold =} \NormalTok{optCutOff2)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
[1] 0.9819483
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confusionMatrix}\NormalTok{(testData2$index_now, predicted2, }\DataTypeTok{threshold =} \NormalTok{optCutOff2)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
     0   1
