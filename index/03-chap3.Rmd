...
Shown below in Figure \ref{fig:plot7} are the sector weights over time for energy stocks. The weighting of energy stocks in the US Equal Weight index is consistently greater than the weighting of energy stocks in the Minimum Volatility index. This could imply that by nature, energy stocks are more volatile than stocks in other industries. 
...
``` {r fig.cap = "Weight of energy stocks by index over time\\label{fig:plot7}", fig.height=4.5, fig.width=6.5, echo=FALSE}
## Energy
...
\clearpage
Shown below in Figure \ref{fig:plot8} are the sector weights over time for financial stocks. The weighting of financial stocks in the US Equal Weight Index is fairly consistent, until a significant drop towards the latter half of 2016. This timing coincides with the Federal Reserve's decision to hike rates for just the second time in a decade, amid positive economic signs [@cox2016]. In the Minimum Volatility Index, the weighting of financials fluctuate quite a bit, and experience a similar drop in weight at that same point in time.

``` {r fig.cap = "Weight of financial stocks by index over time\\label{fig:plot8}", fig.height=2.9, fig.width=6.5, echo=FALSE}
## Financials
...
\newline
Shown below in Figure \ref{fig:plot9} are the sector weights over time for consumer staple stocks. The weighting of consumer staples in the US Equal Weight Index is consistently lower than the weighting of consumer staples in the Minimum Volatility Index. This could imply that consumer staple stocks are less volatile than stocks in other industries. 

``` {r fig.cap = "Weight of consumer staple stocks by index over time\\label{fig:plot9}", fig.height=2.9, fig.width=6.5, echo=FALSE}
## Consumer Staples
...
\clearpage
Shown below in Figure \ref{fig:plot7} are the sector weights over time for energy stocks.
