...
```{r, echo=FALSE, cache=TRUE, message=FALSE, include=FALSE}
load("~/thesis_final/data/usa.Rda")
```
...
``` {r fig.cap = paste("The weights for the constructed US Equal Weight Index are very close to 100%, with just two months exceeding 100%.The minimum weight is 99.54%, while the largest weight is 100.21%. The mean weight is 99.79%."), echo=FALSE}
plot(weight_plot1)
```
...
``` {r fig.cap = paste("The weights for the constructed Minimum Volatility Index are very close to 100%, with no value exceeding 100%. The minimum weight is 99.58%, while the largest weight is 99.99%. The mean weight is 99.76%."), echo=FALSE}
plot(weight_plot2)
```
...
``` {r fig.cap = paste("The correlation between the Constructed US Equal Weight Index monthly returns and EUSA ETF monthly returns is 98.06%."), echo=FALSE}
plot(scatter1)
```
...
``` {r fig.cap = paste("The correlation between the Constructed Minimum Volatility Index monthly returns and USMV ETF monthly returns is 99.07%."), echo=FALSE}
plot(scatter2)
```
